...
def histogram():
    ...
    for i in range(len(counts)):
        ...
        if area_max <= bin_left or area_min >= bin_right:
            ...
            # Partial overlap - we need to split this into segments
            ...
            if overlap_left > bin_left:
                ...
                blue_left_center = bin_left + blue_left_width / 2
                bin_centers.append(blue_left_center)
                bin_widths.append(blue_left_width)
                colors.append('blue')
                bar_counts.append(count)
            
            # Yellow middle portion
            if overlap_right > overlap_left:
                yellow_width = overlap_right - overlap_left
                yellow_center = overlap_left + yellow_width / 2
                bin_centers.append(yellow_center)
                bin_widths.append(yellow_width)
                colors.append('yellow')
                bar_counts.append(count)
            ...
            if bin_right > overlap_right:
                ...
                blue_right_center = overlap_right + blue_right_width / 2
                bin_centers.append(blue_right_center)
                bin_widths.append(blue_right_width)
                colors.append('blue')
                bar_counts.append(count)
    ...
    fig.update_layout(
        ...
        xaxis_title="Gravity Deviation (x10⁻⁸ m/s²)",
        yaxis_title="Frequency",
        xaxis=dict(
            range=[-160, 160],  # Explicitly set the full range
            fixedrange=False,   # Allow zooming if needed
            constrain='domain'  # Constrain to the specified range
        ),
        height=450,  # Keep height fixed
        bargap=0,
        #margin=dict(l=50, r=50, t=120, b=50),  # Keep reasonable margins
        title_x=0.5,  # Center the title
        autosize=True  # Let Plotly auto-size to fit container
    )
